$(document).ready(function()
{
    ...
    var nodeTypes =
    {
        ...
        {
            var attrs = s.attrs;
            var ulArray = ["ul", {"id": attrs.name + "_navigation", "class": "navigation"}];
            var length = attrs.navigation.length;
            for (var i=0; i<length;i++){
                    ulArray.push(["li",callNodeSpec(attrs.navigation[i])]);
            }
            return [
                "div",
        ...
        "instance": function(i)
        {
            ...
            //contents.push(
            var schemaLength = attrs.schema.length;
            for(var i=0; i < schemaLength; i++)
            {
                var col = attrs.schema[i];
                var objectIndex = (col[0]=="text")?1:0;
                var props = {"id": attrs.url + "__" + col[objectIndex].s};
                ...
                else
                {
                    ...
                    contents.push(["div", {"class": props.id + "ColContainer colContainer"}, ["label", {"for": props.id}, (col[1].constructor==Object?col[1].s:col[1]) + ":"], ctl]);
                }
            }//end for
            if (attrs.search)
            {
    ...
    function getDisplayFunc(target, func)
    {
        ...
        return function(json) {
            var jsonDom =json.toDom();
            var element = $(target);
            $.jsonml(jsonDom).appendTo(element);
        };
    }
    ...
    /**
     * This function will check an element to see if it's an array and if it's
     * according to
     *
     */
    function callNodeSpec(node){
       if(node.constructor == Array && node.isNodeSpec()){
           return node.toDom();
       }else{
           return node;
       }
    ...
    {
        return this[0] && this[0].constructor == Object 
            && this[1] && this[1].constructor == Object
            && this[0].s && nodeTypes[this[0].s];
    };
    ...
    Array.prototype.toDom = function()
    {
        if (this.isNodeSpec())
        {
            ...
            else
            {
                ...
                if (this.content)
                {
                    $(dom).appendChild(this.content.toDom ? this.content.toDom() : this.content);
                }
                return dom;
            }
        }
